...
struct MarkdownFormatter {
    /// Enumeration of supported syntax that can be formatted by the MarkdownFormatter.
    ...
    /// Returns a string that has been reformatted based on the given Markdown style.
    /// If the input `string` is already formatted with Markdown syntax,
    /// the function will return a string without the syntax.
    ...
    /// - Warning: This function assumes that an unformatted string is the default state,
    /// so if it fails to find Markdown syntax around the string,
    /// it will return a string with the selected Markdown syntax surrounding it.
    ///
